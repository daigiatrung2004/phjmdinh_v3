...
let wsSymbol = 'btcusdt';
let ws = new WebSocket('wss://stream.binance.com:9443/ws/' + wsSymbol + '@trade');
let lastPrice = null;
let isFlagCreatePlot = false;
let oldMaketSymbol = 'BTC-USDT';
let timeFrame = '15m';
...
function messageHandle(event) {
	...
	$('#current_price').css('color', !lastPrice || lastPrice === price ? 'black' : price > lastPrice ? 'green' : 'red');
	$('#current_price').text(price);
	lastPrice = price;

	callApiAndPlot(getCurrentMarketSymbol(), timeFrame, numberOfDatasets);
}
...
function search() {
	let coin = $('#coin').val();
	let money = $('#money').val();
	MarketSymbol = `${coin}-${money}`.toUpperCase();
	wsSymbol = `${coin}${money}`.toLowerCase();
	$('#binanceMarket').text(MarketSymbol);
	restartSocket();
}
...
function getSMAs() {
	...
	var SMAs = [
		{ name: 'SMA25', value: 25, color: '#E74C3C' },
...
function createButtonGroup() {
	...
	for (var i = 0; i < arrButton.length; i++) {
		document.write(
			'<button class=button id="interval_' +
				arrButton[i].name +
		...
		for (i = 0; i < getMaxNumberOfHighCaps(); i++) {
			document.write(
				'<a id="' +
					...
					i +
					'" ' +
					"onclick=setMarketSymbol(this.text+'-" +
					baseCurrencies[j] +
...
async function getHighCapCoins() {
	var api_url = 'https://api.coinranking.com/v1/public/coins?' + 'limit=' + getMaxNumberOfHighCaps();
...
async function callApiAndPlot(
	marketSymbol = getDefaultMarketSymbol(),
	...
	makePlot(timestamp, marketSymbol, openPrice, highPrice, lowPrice, closePrice, dataSMA, checkboxSMA);
	isFlagCreatePlot = true;
}
...
var arrTrace = [];
function makePlot(timestamp, marketSymbol, openPrice, highPrice, lowPrice, closePrice, dataSMA, checkboxSMA) {
	var trace1 = {
		x: timestamp,
		name: marketSymbol,
		close: closePrice,
	...
	var arrSMA = getSMAs();
	arrTrace.splice(0, arrTrace.length);
